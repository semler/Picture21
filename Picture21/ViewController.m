...
- (void)viewDidLoad {
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth/2-120, screenHeight/2-80, 300, 80)];
    [button setTitle:@"画像を読み込み(名前1枚+写真20枚)" forState:UIControlStateNormal];
    [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [button addTarget:self action:@selector(load) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:button];
    
    UIButton *button2 = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth/2-120, screenHeight/2, 300, 80)];
    [button2 setTitle:@"画像を読み込み(写真21枚)" forState:UIControlStateNormal];
